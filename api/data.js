export default async function handler(req, res) {
  ...
  // Week = Monday to Sunday, displayed as "w/c [Sunday]"
  const now2 = new Date()
  // Get Monday of current week
  const dayOfWeek = now2.getUTCDay() // 0=Sun, 1=Mon...
  const daysToMonday = dayOfWeek === 0 ? -6 : 1 - dayOfWeek
  const monday = new Date(now2)
  monday.setUTCDate(now2.getUTCDate() + daysToMonday)
  monday.setUTCHours(0,0,0,0)
  const sunday = new Date(monday)
  sunday.setUTCDate(monday.getUTCDate() + 6)
  sunday.setUTCHours(23,59,59,999)

  // Display date = Sunday before the Monday (w/c Sunday)
  const displaySunday = new Date(monday)
  displaySunday.setUTCDate(monday.getUTCDate() - 1)
