export default async function handler(req, res) {
  ...
  // Group dates within 3 days of each other into the same batch
  // Use the latest date in each cluster as the canonical drop date
  const ilDates = [...new Set(
    deduped
      .filter(r => r.interview_letter)
      .map(r => r.interview_letter)
  )].sort((a, b) => new Date(a) - new Date(b)) // asc for clustering

  const ilBatches = []
  for (const date of ilDates) {
    const last = ilBatches[ilBatches.length - 1]
    if (last && daysBetween(last, date) <= 3) {
      // Same batch — update to latest date in cluster
      ilBatches[ilBatches.length - 1] = date
    } else {
      ilBatches.push(date)
    }
  }

  // Reverse to descending, take last 15
  const uniqueILDates = ilBatches.reverse().slice(0, 15)
